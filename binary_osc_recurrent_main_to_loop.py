...
#  An "Osc" task (low edge triggered)                    #                
#                                                        #
# MIT LICENCE                                            #
##########################################################
...
def and_fun(t,N_rec,base,base_plot):
    ...
    x_train, y_train,y_train_ ,mask,seq_dur,x,frec= generate_trials(sample_size,mem_gap) #time loop
    ...
    model = Sequential()
    model.add(SimpleRNN(units=N_rec,return_sequences=True, input_shape=(None, 1), kernel_initializer='glorot_uniform',      recurrent_initializer='orthogonal'  ,activation='tanh',use_bias=False))
    #model.add(SimpleRNN(units=N_rec,return_sequences=True, input_shape=(None, 1), kernel_initializer='glorot_uniform',      recurrent_initializer='orthogonal',activation='tanh',use_bias=True,bias_initializer='zeros')) #defaults for the recurrent model!
    ...
    # Saving weigths
    filepath       = base+'/pulse_weights-{epoch:02d}.hdf5'
    #checkpoint    = ModelCheckpoint(filepath, monitor='accuracy')
    #checkpoint     = ModelCheckpoint(filepath)
    callbacks      = [EarlyStoppingByLossVal(monitor='loss', value=0.001, verbose=1), ModelCheckpoint(filepath, monitor='val_loss', save_best_only=False, verbose=1),]
    ...
    print(model.summary())
    plot_model(model, to_file='plots_osc/model.png')
