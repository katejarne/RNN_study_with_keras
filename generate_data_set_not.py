...
def generate_trials(size,mem_gap):
    ...
    for ii in np.arange(sample_size): 
        x_train_[ii, first_in:first_in + stim_dur, 0] = not_seed_A[trial_types[ii], 0]
        x_train[ii, first_in:first_in + stim_dur, 0] = not_seed_A[trial_types[ii], 0]
        #x_train[ii, first_in:first_in + stim_dur, 0]  = signal.convolve(x_train_[ii, first_in:first_in + stim_dur, 0], win, mode='same') / sum(win) 
        y_train[ii, out_t + var_delay[ii]:, 0]        = not_y[trial_types[ii]]
    ...
    print("--- %s seconds to generate Not dataset---" % (time.time() - start_time))
...
for ii in np.arange(10):
    ...
    figname = "plots_not/data_set_not_sample.png" 
